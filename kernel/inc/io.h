...
static __inline void insb(unsigned short int __port, void *addr, unsigned long int __count)
{
    __asm__ __volatile__("cld ; rep ; insb" : "=D"(addr), "=c"(__count)
                         : "d"(__port), "0"(addr), "1"(__count));
}

static __inline void insw(unsigned short int __port, void *addr, unsigned long int __count)
{
    __asm__ __volatile__("cld ; rep ; insw" : "=D"(addr), "=c"(__count)
                         : "d"(__port), "0"(addr), "1"(__count));
}

static __inline void insl(unsigned short int __port, void *addr, unsigned long int __count)
{
    __asm__ __volatile__("cld ; rep ; insl" : "=D"(addr), "=c"(__count)
                         : "d"(__port), "0"(addr), "1"(__count));
}

static __inline void outsb(unsigned short int __port, const void *addr, unsigned long int __count)
{
    __asm__ __volatile__("cld ; rep ; outsb" : "=S"(addr), "=c"(__count)
                         : "d"(__port), "0"(addr), "1"(__count));
}

static __inline void outsw(unsigned short int __port, const void *addr, unsigned long int __count)
{
    __asm__ __volatile__("cld ; rep ; outsw" : "=S"(addr), "=c"(__count)
                         : "d"(__port), "0"(addr), "1"(__count));
}

static __inline void outsl(unsigned short int __port, const void *addr, unsigned long int __count)
{
